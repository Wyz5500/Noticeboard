...
浏览器只持久化当前演示身份和视觉偏好；任务、评论、评论草稿、评论编辑草稿、权限状态和其他服务器数据不进入浏览器持久化存储，评论创建与编辑草稿仅按身份、任务和适用的评论保存在当前页面内存中。客户端“我的任务”归属只依据生命周期活动，不把评论创建、编辑或删除视为任务归属变更。秘密不得写入浏览器存储。用户内容只经 `textContent` 或安全节点工厂进入 DOM，不能通过 HTML 字符串拼接进入页面。
