...
`tasks`、`identity`、`health` 是功能模块。业务模块内部遵循：
...
写用例为 `CreateTask`、`ActOnTask`、`ResetDemoTasks`；读用例为 `ListTasks`、`GetTask`、`ListDemoActors`。列表与统计是真实只读需求，因此通过 `TaskQueryPort` 返回投影；领域恢复和写入通过聚合语义的 `TaskRepositoryPort`。
...
迁移创建 `accounts`、`tasks`、`task_events`，用外键、每任务事件顺序主键、创建时间排序索引和乐观版本列保护数据。事件同时保存操作人的 ID、姓名与角色快照，账户资料变化不会重写历史。生产配置永久使用 `synchronize: false`。部署 seed 幂等写入三个演示身份，只在任务表为空时于单一事务中初始化十二项任务；显式 reset 才替换任务数据。

HTTP 使用 URI 版本 `/api/v1`。稳定枚举、字段、状态码与 demo-only 身份头以 `/api/openapi.json` 为唯一字段契约；本文不复制 DTO 字段以避免双重真相。未来正式认证只替换身份适配器。

## 前端边界

页面启动时并行读取身份和全部任务，随后在内存执行 hash 路由、范围、状态、搜索和统计。命令成功后重新读取任务；网络失败和乐观冲突沿用 toast/表单错误语言并重新同步。`RequestGate` 阻止同一命令重复提交。用户内容只经 `textContent` 或安全节点工厂进入 DOM。
