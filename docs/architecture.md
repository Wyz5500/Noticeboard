...
迁移创建 `accounts`、`tasks`、`task_events`，用外键、每任务事件顺序主键、创建时间排序索引和乐观版本列保护数据。事件同时保存操作人的 ID、姓名与角色快照，账户资料变化不会重写历史。生产配置永久使用 `synchronize: false`。部署 seed 幂等写入三个演示身份，只在任务表为空时于单一事务中初始化四项任务；显式 reset 才替换任务数据。
