...
人类输出在每个输出块渲染前读取目标流（stdout 或 stderr）的当前终端宽度。顶部、底部及表格内部所有分割线均不超过该宽度；交互确认使用其输出流宽度。未知或无效宽度（包括通常的管道或重定向）保留原有按内容排版，不依赖 `COLUMNS` 环境变量。

表格能放下时保留原布局；超宽时缩减列宽，在单元格内按完整字素主动折行，并补齐其他列的续行，保留全部字段内容。中文、组合字符与 emoji 按显示宽度计算。如果终端连每列一个完整字素及列间隔都容纳不了，改为提示扩大窗口或使用 `--json`，不改变命令成功状态或退出码。仅 1 列时提示使用英文，以免中文本身超宽。帮助、手册、详情、时间线、成功消息、错误和确认提示的正文同样按完整字素主动折行，保留原有段落换行；有足够空间时续行保留原缩进，缩进过宽时按整行折行。如果宽度连单个字素都无法容纳，则显示扩大窗口或使用 `--json` 的提示。已输出内容不随窗口缩放重排；JSON 不受终端宽度影响。
